...
class TestDialogueManager:
    ...
    def test_target_dialog_state_management(self):
        """Correctly sets the dialogue state based on the target_dialog_state"""
        context = create_context('domain', 'intent')
        result = self.dm.apply_handler(context, target_dialog_state='intent_entity_2')
        assert result['dialogue_state'] == 'intent_entity_2'
