...
def test_nlp_greet(diaglogflow_nlp):
    query1 = 'hi there'
    assert diaglogflow_nlp.process(query1)['intent'] == 'default_welcome_intent_usersays_en'


def test_nlp_i_can_code_in_en(diaglogflow_nlp):
    query1 = 'I know Python'
    assert diaglogflow_nlp.process(query1)['intent'] == 'i_can_code_in_a_language_usersays_en'
...
def test_nlp_i_can_code_in_es(diaglogflow_nlp):
    ...
    assert diaglogflow_nlp.process(query1)['intent'] == 'i_can_code_in_a_language_usersays_es'
    entity = diaglogflow_nlp.process(query1)['entities']
