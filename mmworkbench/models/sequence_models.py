...
from . import query_features, sequence_tagging, helpers
...
class MemmModel(BaseEntityRecognizer):
    ...
    def _extract_query_features(self, query):
        ...
        for name, kwargs in self.feat_specs.items():
            feat_extractor = helpers.get_feature_extractor('query', name)(**kwargs)
    ...
    def fit(self, labeled_queries, entity_types=None, verbose=False):
        ...
        for idx, query in enumerate(labeled_queries):
            ...
            tags = sequence_tagging.get_tags_from_entities(query, self._tag_scheme)
            query_groups = [idx for _ in tags]

            if len(features) > 0:
                # Set the special feature for the identity of the previous tag.
                features[0]['prev_tag'] = sequence_tagging.START_TAG
    ...
    def _predict(self, query, verbose=False):
        ...
        if len(query_features) == 0:
            return [], {}
        query_features[0]['prev_tag'] = sequence_tagging.START_TAG

        predicted_tags = []
        predicted_probs = []
        # reference_tags = sequence_tagging.get_tags_from_entities(query, self._tag_scheme)
        # reference_probs = []
        confidence_data = []
        active_features = []
        all_log_probs = []
        ...
        for i, features in enumerate(query_features):
            ...
            #     out_tag = '|'.join([sequence_tagging.O_TAG, '', sequence_tagging.O_TAG, ''])
        ...
        entities = sequence_tagging.get_entities_from_tags(query, predicted_tags)
