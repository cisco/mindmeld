...
class Tokenizer(object):
    ...
    def tokenize(self, text, keep_special_chars=True):
        """
        This method tokenizes input text, normalizes the token text and returns normalized tokens.
    ...
    def get_char_index_map(self, raw_text, normalized_text):
        """
        This method generates character index mapping from normalized query to raw query. The
        entity model always operates on normalized query during NLP processing but for entity
        output we need to generate indexes based on raw query.
